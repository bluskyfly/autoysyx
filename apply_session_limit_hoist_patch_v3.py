"""v3 of the session-limit hoist patch.

v2 step 1 used [^)]*? which can't traverse the nested parens inside the
regex pattern stored in _SESSION_LIMIT_RE = re.compile(...). v3 uses
lazy .*? with DOTALL instead.

Idempotent: re-runs are no-ops with a clear print.

Run from autoysyx repo root:
    python3 apply_session_limit_hoist_patch_v3.py
"""
import re
...
# ----------------------------------------------------------------------
# 1. Replace the _SESSION_LIMIT_RE assignment block. Lazy DOTALL match
#    handles nested parens inside the compiled regex pattern.
# ----------------------------------------------------------------------
re_block_pat = re.compile(
    r'_SESSION_LIMIT_RE\s*=\s*re\.compile\(.*?re\.IGNORECASE\s*,?\s*\)',
    re.DOTALL,
)
new_re_block = (
...
if "_SESSION_LIMIT_TRIGGERS" in src:
    ...
    if not m:
        raise SystemExit(
            "step 1 FAILED: could not find _SESSION_LIMIT_RE block.\n"
            "Inspect:  sed -n '50,75p' orchestrator/worker.py"
        )
...
if "any(t in low for t in _SESSION_LIMIT_TRIGGERS)" in src:
    ...
    if not m:
        raise SystemExit(
            ...
            "Inspect:  grep -n 'session limit' orchestrator/worker.py"
        )
...
if hoist_marker in src:
    ...
    injection = (
        m.group(0)
        + f'\n{indent}# {hoist_marker}. Claude sometimes prints the limit\n'
        + f'{indent}# notice as plain text (non-JSON), which would bypass\n'
        + f'{indent}# the post-parse detection further down. Check raw\n'
        + f'{indent}# stdout+stderr first.\n'
